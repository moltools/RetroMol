...
def ensure_hmm3_pressed(hmm3_file: str | Path) -> None:
    """ Run `hmmpress` on the packaged HMM3 profile once, if its binary index files aren't there yet. """
    hmm3_file = Path(hmm3_file)
    if all((hmm3_file.parent / f"{hmm3_file.name}.{ext}").exists() for ext in ("h3f", "h3i", "h3m", "h3p")):
        return
    subprocess.run(["hmmpress", str(hmm3_file)], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)


def run_hmmscan(hmm_file: str | Path, fasta_file: str | Path, out_file: str | Path) -> None:
    """ Run HMMER3's `hmmscan` on a fasta file against a (pressed) HMM database. """
    with open(out_file, "w") as out:
        subprocess.run(["hmmscan", str(hmm_file), str(fasta_file)], check=True, stdout=out)


def run_hmmpfam2(hmm_file: str | Path, fasta_file: str | Path, out_file: str | Path) -> None:
    """ Run HMMER2's `hmmpfam2` on a fasta file against an HMM database. """
    with open(out_file, "w") as out:
        subprocess.run(["hmmpfam2", str(hmm_file), str(fasta_file)], check=True, stdout=out)
