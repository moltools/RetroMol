...
fingerprinting) enqueues its work here rather than running it in-thread. See
routes/jobs.py and routes/discovery.py for the task functions themselves; this
module only holds the queue and the bridge that lets a synchronous Flask handler
use it without any frontend contract change.
"""
...
def enqueue_job(fn: Callable[..., Any], *args: Any) -> Job:
    """
    ...
    """
    return queue.enqueue(fn, *args, job_timeout=_HEAVY_JOB_RQ_TIMEOUT_SECONDS)


def enqueue_and_wait(
    fn: Callable[..., Any], *args: Any, timeout: int = HEAVY_JOB_WAIT_TIMEOUT_SECONDS
) -> Any:
    """
